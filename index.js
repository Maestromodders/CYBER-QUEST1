/**
 * CYBER_QUEST WhatsApp Bot - Main Entry Point
 */
...
async function startBot() {
  ...
  if (config.sessionID && config.sessionID.startsWith('KnightBot!')) {
    try {
      ...
      console.log('📡 Session : 🔑 Retrieved from CYBER_QUEST Session');

    } catch (e) {
      console.error('📡 Session : ❌ Error processing CYBER_QUEST session:', e.message);
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log('⚡ CYBER_QUEST is online and ready!\n');
